...
--
-- Hôte : 127.0.0.1
-- Généré le : mar. 27 sep. 2022 à 17:54
...
--
-- Doublure de structure pour la vue `rapport_presenec`
-- (Voir ci-dessous la vue réelle)
--
CREATE TABLE `rapport_presenec` (
`id` bigint(21)
,`noms` varchar(156)
,`classe` varchar(50)
,`option_` varchar(50)
...
(5, 'jered', 'jered', 'jered', 'M', '1984-06-24', 'jered', 'jered', 'etudiant', 'himbi 2', '+243 997604486', 0x39343335363533302e6a7067, '2022/jer/005');
...
(2, '2022/moi/004', '12:35:19 PM', '2022-09-25', '1'),
(3, '2022/Win/003', '15:46:33 PM', '2022-09-27', '1'),
(4, '2022/moi/004', '15:46:46 PM', '2022-09-27', '1'),
(5, '2022/jer/005', '17:44:57 PM', '2022-09-27', '1');
...
--
-- Structure de la vue `rapport_presenec`
--
DROP TABLE IF EXISTS `rapport_presenec`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `rapport_presenec`  AS SELECT row_number() over ( order by `t_eleve`.`Nom` desc) AS `id`, concat(`t_eleve`.`Nom`,'   ',`t_eleve`.`Postnom`,'   ',`t_eleve`.`Prenom`) AS `noms`, `t_classe`.`designation` AS `classe`, `t_option`.`designation` AS `option_`, `t_annee`.`designation` AS `annee`, `t_presence`.`LOGDATE` AS `LOGDATE`, CASE WHEN `t_presence`.`status` is null THEN 0 ELSE 1 END AS `statut`, `t_presence`.`TIMEIN` AS `heure` FROM (((((`t_eleve` join `t_presence` on(`t_eleve`.`matricule` = `t_presence`.`matriculle_eleve`)) join `t_inscription` on(`t_inscription`.`ref_et` = `t_eleve`.`id`)) join `t_classe` on(`t_classe`.`id` = `t_inscription`.`ref_classe`)) join `t_option` on(`t_option`.`id` = `t_classe`.`ref_option`)) join `t_annee` on(`t_annee`.`id` = `t_inscription`.`ref_annee`))  ;
